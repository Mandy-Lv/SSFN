...
class DataCollatorForASTE:
    def __init__(self, tokenizer, max_seq_length, pos2id=None, pos_mapper=None):
    ...
    def tokenizer_function(self, examples):
        ...
        pos_ids = []
        for i, postags in enumerate(postag):
            try:
            ...
            if self.pos_mapper is not None:
                postags = self.pos_mapper(postags)
            ...
            for token_idx in range(length):
                ...
                if word_idx is None:
                    subword_pos_id.append(self.pos2id['PAD'])
                else:
                    if word_idx < len(postags):
                        subword_pos_id.append(self.pos2id.get(postags[word_idx], self.pos2id['PAD']))
                    else:
                        subword_pos_id.append(self.pos2id['PAD'])

            pos_ids.append(subword_pos_id)
...
class ASTEDataModule(pl.LightningDataModule):
    ...
    def map_pos_tag(self, tag):
        granularity = getattr(self.args, "pos_granularity", "fine")

        if granularity == "fine":
            return tag

        if granularity == "coarse5":
            noun_tags = {"NN", "NNS", "NNP", "NNPS", "PRP", "PRP$", "WP"}
            verb_tags = {"VB", "VBD", "VBG", "VBN", "VBP", "VBZ", "MD"}
            adj_adv_tags = {"JJ", "JJR", "JJS", "RB", "RBR", "RBS", "WRB", "RP"}
            func_tags = {"DT", "PDT", "WDT", "IN", "CC", "TO", "EX", "POS", "UH"}
            ...
            if tag in noun_tags:
                ...
                return "ADJ_ADV"
            elif tag in func_tags:
                return "FUNC"
            else:
                return "OTHER"

        if granularity == "mid17":
            mapping = {
                "NN": "NOUN", "NNS": "NOUN",
    ...
    def map_postags(self, postags):
        return [self.map_pos_tag(tag) for tag in postags if isinstance(tag, str)]
    def load_dataset(self):
        ...
        for file_name in [train_file_name, dev_file_name, test_file_name]:
            ...
            for example in data:
                ...
                mapped_postags = self.map_postags(postags)
                self.pos_tags.update(mapped_postags)
