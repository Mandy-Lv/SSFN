...
from model.SSFN_model import SSFNModel
...
class ASTE(pl.LightningModule):
    def __init__(self, hparams, data_module):
        ...
        self.config.seq2mat = hparams.seq 
        self.config.num_d = self.hparams.num_d 
        self.config.num_pos_tags = len(self.data_module.pos2id) 
        self.config.pos_embed_dim = self.hparams.pos_embed_dim
        self.config.fusion_type = hparams.fusion_type  
        self.model = SSFNModel.from_pretrained(self.hparams.model_name_or_path, config=self.config)        
    ...
    def load_model(self):
        ...
        self.model = SSFNModel.from_pretrained(dir_name)

    def forward(self, input_ids, attention_mask, ids, text, adj_pack,
                start_label_masks, end_label_masks, length, pos_ids,
                t_start_labels=None, t_end_labels=None, o_start_labels=None,
                o_end_labels=None, table_labels_S=None, table_labels_E=None,
                pairs_true=None):
        inputs = {
            'input_ids': input_ids,
            ...
            'text': text,
            'adj_pack': adj_pack,
            'start_label_masks': start_label_masks,
            'end_label_masks': end_label_masks,
            'length': length,
            'pos_ids': pos_ids,
            't_start_labels': t_start_labels,
            't_end_labels': t_end_labels,
            'o_start_labels': o_start_labels,
            'o_end_labels': o_end_labels,
    ...
    def training_step(self, batch, batch_idx):
        ...
        outputs = self(
            ...
            t_start_labels=batch.get('t_start_labels'),
            t_end_labels=batch.get('t_end_labels'),
            o_start_labels=batch.get('o_start_labels'),
            o_end_labels=batch.get('o_end_labels'),
            table_labels_S=batch.get('table_labels_S'),
            table_labels_E=batch.get('table_labels_E'),
            pairs_true=batch.get('pairs_true')
        )
        fused_table = outputs.get('fused_table', None)
    ...
    def add_model_specific_args(parser):
        ...
        parser.add_argument("--model_name_or_path", default='code/bert-base-uncased', type=str)
        parser.add_argument('--prefix', type=str, default="data/aste_data_bert/V2/",)
        parser.add_argument('--dataset', type=str, default="14res", help='dataset')
        ...
        parser.add_argument("--num_t", default=2, type=int)
        parser.add_argument("--seq", default='tensorcontext', type=str)
        ...
        parser.add_argument("--seq2mat", type=str, default='tensorcontext', choices=['none','tensor','context','tensorcontext'])
        parser.add_argument("--num_d", type=int, default=64)
        parser.add_argument("--pos_embed_dim", default=64, type=int, help="Dimension of POS pair embedding")
        parser.add_argument("--pos_granularity", default="mid17", type=str,
                    choices=["coarse5", "mid17", "fine"],
                    help="Granularity of POS tags for PARE")
        parser.add_argument("--fusion_type", default='add', type=str, choices=['add', 'mul'], help="Fusion type for table and pos_quant_matrix")  
        return parser
...
def main():
    ...
    run_seed: int = random.randint(0, 2**32 - 1)
